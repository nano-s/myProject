...
if uploaded_file:
    ...
    st.image(cv2.bitwise_and(image, image, mask=glow_mask), caption="📍 บริเวณที่ควรล้างมือเพิ่มเติม", use_column_width=True)
    ...
    for name, zone_mask in zones.items():
        ...
        if zone_glow > 500:  # ปรับ threshold ตามความเหมาะสม
